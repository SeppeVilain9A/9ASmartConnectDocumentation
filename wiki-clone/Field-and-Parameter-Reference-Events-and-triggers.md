...
For each matched processor the service calls `NANProcessorEventTable::createEvent()`, writing one event row (source table + record + document Id, Status = *Waiting*). From there the document follows the normal path: if the processor uses a queue it is enqueued and run by the retry engine / batch; an event with *Skip queue execution* (or no queue) runs the handler and connector immediately. See [Queue reference](Field-and-Parameter-Reference-Queue-fields) and [Core concepts](Getting-Started-Key-concepts).
