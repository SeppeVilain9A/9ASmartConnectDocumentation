...
When a processor uses a **9A Raptor DWH** connector, the processor form gains a **9A Raptor DWH** tab and a **Raptor connector setup** button (next to the *Regex* button used for [PO recognition](E-Invoicing-and-EDI-Purchase-order-recognition)).
...
| **Value** | The value to match — usually a [placeholder](Functional-Guide-Placeholders) such as `%InvoiceIdMYSUPPLY%` that is expanded from the payload. |
