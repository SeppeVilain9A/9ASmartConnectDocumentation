...
These are the fields the question *"why did new fields just appear?"* is about. Each **driver** below, when set, shows or enables additional controls. Fields gated by a *global* parameter (in [Parameters](Field-and-Parameter-Reference-Global-parameters)) do not appear at all until that parameter is switched on.
...
Full mechanics — dispatcher, work items, pessimistic claim: [Multithreading & work dispatch](Technical-Reference-Multithreading-and-work-dispatch).
...
>
> The *Events* group here only *declares* the business process. For the full picture — every out-of-the-box trigger, the difference between *All accounts* and the *Business process config* table, and how invoice-type filtering works — see [Events & triggers — when a document fires](Field-and-Parameter-Reference-Events-and-triggers).
